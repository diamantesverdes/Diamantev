                          ...
                          {galleryPlants.map(p => (
                            <div key={p.id} className="gallery-item" style={{ position: 'relative' }}>
                              ...
                              {p.shared_visible && (
                                <button
                                  type="button"
                                  title="Compartida — toca para quitarla del link"
                                  onClick={e => { e.stopPropagation(); markSelectedShared(new Set([p.id]), false) }}
                                  style={{ position: 'absolute', top: 4, right: 4, zIndex: 2, background: 'rgba(255,255,255,0.85)', border: '1px solid #ccc', borderRadius: 6, padding: '1px 3px', cursor: 'pointer' }}
                                >📤</button>
                              )}
                              {(p.printed_at || p.pdf_generated_at || p.whatsapp_shared_at) && (
                                <div style={{ position: 'absolute', top: 4, right: p.shared_visible ? 28 : 4, zIndex: 2, display: 'flex', gap: 2, background: 'rgba(255,255,255,0.85)', border: '1px solid #ccc', borderRadius: 6, padding: '1px 3px' }}>
                                  {p.printed_at && <span title={`Impresa: ${new Date(p.printed_at).toLocaleDateString()}`}>🏷️</span>}
                                  {p.pdf_generated_at && <span title={`En PDF: ${new Date(p.pdf_generated_at).toLocaleDateString()}`}>📄</span>}
                                  {p.whatsapp_shared_at && <span title={`Enviada por WhatsApp: ${new Date(p.whatsapp_shared_at).toLocaleDateString()}`}>📲</span>}
                                </div>
                              )}
                                ...
                                <table className="invoice-table" style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
                                  ...
                                  <tbody>
                                    {lineasConProrrateo.map(c => (
                                      <tr key={c.id}>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>{c.image_url ? <img src={c.image_url} alt={c.plant_name} style={{ width: 32, height: 32, objectFit: 'cover', borderRadius: 4 }} /> : '—'}</td>
                                        <td style={{ padding: 4, border: '1px solid #ddd' }}>
