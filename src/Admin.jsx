...
export default function Admin() {
  ...
  const [ventaLineForm, setVentaLineForm] = useState({ plant_id: '', quantity: '', unit_price: '' })
  ...
  function addLineToVentaLote(e) {
    e.preventDefault()
    if (!ventaLineForm.plant_id || !ventaLineForm.quantity) {
      alert('Selecciona una planta y la cantidad')
      return
    }
    ...
    setVentaLoteLines(prev => [...prev, {
      ...ventaLineForm,
      plant_name: plant ? plant.name : '',
      unit_price: ventaLineForm.unit_price !== '' ? ventaLineForm.unit_price : (plant ? plant.price : 0),
    }])
    setVentaLineForm({ plant_id: '', quantity: '', unit_price: '' })
  }
  ...
  async function saveVentaLote() {
    if (ventaLoteLines.length === 0) { alert('Agrega al menos una planta a la venta'); return }
    ...
    for (const line of ventaLoteLines) {
      ...
      const quantity = Number(line.quantity)
      const unit_price = Number(line.unit_price) || (plant ? Number(plant.price) : 0)
      const { error } = await supabase.from('decrementos').insert({
        plant_id: line.plant_id, plant_name: line.plant_name, quantity, motivo: 'Venta', unit_price, lote_id: lote.id,
      })
      if (error) { alert('Error al guardar una de las plantas: ' + error.message); continue }
      if (plant) await supabase.from('plants').update({ stock: Math.max(0, plant.stock - quantity) }).eq('id', plant.id)
    }
    setVentaLoteCliente('')
    setVentaLoteLines([])
  ...
  async function addPlantToVentaLote(loteId) {
    ...
    const unit_price = Number(ventaLineForm.unit_price) || (plant ? Number(plant.price) : 0)
    const { error } = await supabase.from('decrementos').insert({
      plant_id: ventaLineForm.plant_id, plant_name: plant ? plant.name : '', quantity, motivo: 'Venta', unit_price, lote_id: loteId,
    })
  ...
  async function downloadLabelsPDF(list) {
    ...
    try {
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        doc.setFillColor(161, 102, 94)
        doc.rect(x, y, labelW, bannerH, 'F')
        doc.setTextColor(255, 255, 255)
        doc.setFontSize(11)
        doc.text(p.name, x + labelW / 2, y + bannerH / 2 + 3, { align: 'center', maxWidth: labelW - 4 })
      }
                ...
                <>
                  ...
                  {ventaLoteBuilderOpen && (
                    <div className="admin-form" style={{ marginBottom: 12 }}>
                      <h3>Nueva venta</h3>
                      <input placeholder="Cliente (opcional)" value={ventaLoteCliente} onChange={e => setVentaLoteCliente(e.target.value)} />
                      ...
                      />
                      <select value={ventaLineForm.plant_id} onChange={e => setVentaLineForm({ ...ventaLineForm, plant_id: e.target.value })}>
                      ...
                      <input placeholder="Precio unitario (opcional)" type="number" step="0.01" value={ventaLineForm.unit_price} onChange={e => setVentaLineForm({ ...ventaLineForm, unit_price: e.target.value })} />
                      <button type="button" onClick={addLineToVentaLote}>➕ Agregar planta a la venta</button>
                          ...
                          {ventaLoteLines.map((l, i) => (
                            <div key={i} className="admin-item">
                              <div className="admin-item-info">
                                ...
                                <span>Cant: {l.quantity} — $ {Number(l.unit_price).toFixed(2)} c/u</span>
                  ...
                  <form className="admin-form" onSubmit={addDecremento}>
                    <h3>Registrar decremento manual (mermas)</h3>
                    <select className="gallery-select" value={decPlantCategory} onChange={e => setDecPlantCategory(e.target.value)}>
                      <option value="all">Todas las categorías</option>
                      {categories.map(c => <option key={c.id} value={c.id}>{c.emoji} {c.name}</option>)}
                    </select>
                              ...
                              <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
                                ...
                                <tbody>
                                  {lineasConProrrateo.map(d => (
                                    <tr key={d.id}>
                                      <td style={{ padding: 4, border: '1px solid #ddd' }}>{d.plant_name}</td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>{d.quantity}</td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${Number(d.unit_price || 0).toFixed(2)}</td>
                                ...
                                <tfoot>
                                  <tr><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd' }}>Subtotal</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${subtotal.toFixed(2)}</td></tr>
                                  <tr><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 1</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><input type="number" step="0.01" defaultValue={lote.envio1 || 0} onBlur={e => updateVentaLoteExtra(lote, 'envio1', e.target.value)} style={{ width: 80 }} /></td></tr>
                                  <tr><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 2</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><input type="number" step="0.01" defaultValue={lote.envio2 || 0} onBlur={e => updateVentaLoteExtra(lote, 'envio2', e.target.value)} style={{ width: 80 }} /></td></tr>
                                  <tr><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd' }}>Varios</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><input type="number" step="0.01" defaultValue={lote.varios || 0} onBlur={e => updateVentaLoteExtra(lote, 'varios', e.target.value)} style={{ width: 80 }} /></td></tr>
                                  <tr style={{ background: '#f3ecdd' }}><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd' }}><strong>Total</strong></td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><strong>${totalLote.toFixed(2)}</strong></td></tr>
                                </tfoot>
                              </table>
                            </div>
                            ...
                            {addToVentaLoteId === lote.id ? (
                              <div className="admin-form" style={{ marginTop: 8 }}>
                                ...
                                <div className="admin-item-actions">
                                  ...
                                  <button type="button" onClick={() => { setAddToVentaLoteId(null); setVentaLineForm({ plant_id: '', quantity: '', unit_price: '' }) }}>Cancelar</button>
                                </div>
                              </div>
                            ) : (
                              ...
                              <div style={{ overflowX: 'auto', marginTop: 8 }}>
                                <table className="invoice-table" style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
                                    ...
                                    {lineasConProrrateo.map(c => (
                                      <tr key={c.id}>
                                        ...
                                        <td style={{ padding: 4, border: '1px solid #ddd' }}>
                                          <input defaultValue={c.plant_name} onBlur={e => updateCompraField(c, 'plant_name', e.target.value)} style={{ width: 90 }} />
                                        </td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                          <input type="number" defaultValue={c.quantity} onBlur={e => updateCompraField(c, 'quantity', e.target.value)} style={{ width: 45 }} />
                                        </td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>
                                          <input type="number" step="0.01" defaultValue={c.unit_cost} onBlur={e => updateCompraField(c, 'unit_cost', e.target.value)} style={{ width: 60 }} />
                                        </td>
                                  ...
                                  <tfoot>
                                    ...
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 1</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><input type="number" step="0.01" defaultValue={lote.envio1 || 0} onBlur={e => updateLoteExtra(lote, 'envio1', e.target.value)} style={{ width: 80 }} /></td></tr>
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 2</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><input type="number" step="0.01" defaultValue={lote.envio2 || 0} onBlur={e => updateLoteExtra(lote, 'envio2', e.target.value)} style={{ width: 80 }} /></td></tr>
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Varios</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><input type="number" step="0.01" defaultValue={lote.varios || 0} onBlur={e => updateLoteExtra(lote, 'varios', e.target.value)} style={{ width: 80 }} /></td></tr>
          ...
          {plants.filter(p => selectedLabels.has(p.id)).map(p => (
            ...
            >
              <span
                ...
                style={{ background: '#a1665e', color: '#fff', fontWeight: 'bold', textAlign: 'center', padding: '2mm 1mm', fontSize: '11pt', lineHeight: 1.1 }}
              >
                {p.name}
              </span>
              {p.image_url
