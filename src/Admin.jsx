                  ...
                  <div className="admin-sheet-overlay" onClick={() => { setPhotoModalPlantId(null); setPhotoModalMenuOpen(false); setPhotoModalSection(null) }}>
                    <div className="free-note-modal" onClick={e => e.stopPropagation()} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100%', height: '100%', maxWidth: 'none', maxHeight: 'none', borderRadius: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                      ...
                      {photoModalMenuOpen && (
                        ...
                        >
                          <button type="button" style={{ textAlign: 'left', padding: '12px 16px' }} onClick={() => { setPhotoModalSection('comercio'); setPhotoModalMenuOpen(false) }}>🏷️ Comercio</button>
              ...
              {view === 'pedidos' && (
                <>
                  <form className="admin-form" onSubmit={addDecremento}>
                    ...
                    {decForm.motivo === 'Venta' && (
                      <input
                        placeholder="Precio unitario de venta"
                        type="number"
                        step="0.01"
                        value={decForm.unit_price}
                        onChange={e => setDecForm({ ...decForm, unit_price: e.target.value })}
                      />
                    )}
                    {decForm.motivo === 'Otro' && (
                                  ...
                                  <tbody>
                                    {lineasConProrrateo.map(c => (
                                      <tr key={c.id}>
                                        ...
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                          {c.status === 'pedido' && (
                                            <button onClick={() => markCompraPagada(c)} disabled={approvingIds.includes(c.id)} style={{ fontSize: 11 }}>
                                              {approvingIds.includes(c.id) ? '...' : 'Pagar'}
                                            </button>
                                          )}
                                          {c.status === 'pagado' && (
                                            <button onClick={() => markCompraRecibida(c)} disabled={approvingIds.includes(c.id)} style={{ fontSize: 11 }}>
                                              {approvingIds.includes(c.id) ? '...' : 'Recibir'}
                                            </button>
                                          )}
                                          {c.status === 'recibido' && <span style={{ fontSize: 11 }}>✓ recibido</span>}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
