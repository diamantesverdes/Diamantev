...
export default function Admin() {
  ...
  const [loteAddingNewPlant, setLoteAddingNewPlant] = useState(false)
  ...
  function PlantPicker({ list, selectedId, onSelect }) {
    return (
      <div style={{ maxHeight: 180, overflowY: 'auto', border: '1px solid #ccc', borderRadius: 8, marginBottom: 8 }}>
                ...
                <>
                  ...
                  <div className="admin-list">
                    {ventaLotes.map(lote => {
                      ...
                      return (
                        <div key={`vl-${lote.id}`} className="admin-item lote-group">
                          <div className="admin-item-info">
                              ...
                              <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
                                <thead>
                                  <tr style={{ background: '#f3ecdd' }}>
                                    ...
                                    <th style={{ padding: 4, border: '1px solid #ddd' }}>Motivo</th>
                                    ...
                                    <th style={{ padding: 4, border: '1px solid #ddd' }}>Estado</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {lineasConProrrateo.map(d => (
                                    <tr key={d.id}>
                                      <td style={{ padding: 4, border: '1px solid #ddd' }}>
                                        {editingV ? <input defaultValue={d.plant_name} onBlur={e => updateDecrementoField(d, 'plant_name', e.target.value)} style={{ width: 90 }} /> : d.plant_name}
                                      </td>
                                      <td style={{ padding: 4, border: '1px solid #ddd' }}>
                                        {editingV ? (
                                          <select defaultValue={d.motivo} onChange={e => updateDecrementoField(d, 'motivo', e.target.value)}>
                                            <option value="Venta">Venta</option>
                                            <option value="Dañada / Muerta">Dañada / Muerta</option>
                                            <option value="Uso propio">Uso propio</option>
                                            <option value="Regalo">Regalo</option>
                                      ...
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                        {editingV ? <input type="number" defaultValue={d.quantity} onBlur={e => updateDecrementoField(d, 'quantity', e.target.value)} style={{ width: 45 }} /> : d.quantity}
                                      </td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>
                                        {editingV ? <input type="number" step="0.01" defaultValue={d.unit_price || 0} onBlur={e => updateDecrementoField(d, 'unit_price', e.target.value)} style={{ width: 60 }} /> : `$${Number(d.unit_price || 0).toFixed(2)}`}
                                      </td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${d._value.toFixed(2)}</td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${d._prorated.toFixed(2)}</td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${d._lineTotal.toFixed(2)}</td>
                                      <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                        <StatusChecklist
                                ...
                                <tfoot>
                                  <tr><td colSpan={5} style={{ padding: 4, border: '1px solid #ddd' }}>Subtotal</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${subtotal.toFixed(2)}</td></tr>
                                  <tr><td colSpan={5} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 1</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>{editingV ? <input type="number" step="0.01" defaultValue={lote.envio1 || 0} onBlur={e => updateVentaLoteExtra(lote, 'envio1', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.envio1 || 0).toFixed(2)}`}</td></tr>
                                  <tr><td colSpan={5} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 2</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>{editingV ? <input type="number" step="0.01" defaultValue={lote.envio2 || 0} onBlur={e => updateVentaLoteExtra(lote, 'envio2', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.envio2 || 0).toFixed(2)}`}</td></tr>
                                  <tr><td colSpan={5} style={{ padding: 4, border: '1px solid #ddd' }}>Varios</td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>{editingV ? <input type="number" step="0.01" defaultValue={lote.varios || 0} onBlur={e => updateVentaLoteExtra(lote, 'varios', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.varios || 0).toFixed(2)}`}</td></tr>
                                  <tr style={{ background: '#f3ecdd' }}><td colSpan={5} style={{ padding: 4, border: '1px solid #ddd' }}><strong>Total</strong></td><td colSpan={3} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><strong>${totalLote.toFixed(2)}</strong></td></tr>
                                </tfoot>
                              </table>
                            </div>
                            ...
                            {addToVentaLoteId === lote.id ? (
                              <div className="admin-form" style={{ marginTop: 8 }}>
                                ...
                                <input placeholder="Precio unitario (0 si es merma)" type="number" step="0.01" value={ventaLineForm.unit_price} onChange={e => setVentaLineForm({ ...ventaLineForm, unit_price: e.target.value })} />
                              ...
                              <button type="button" onClick={() => setAddToVentaLoteId(lote.id)} style={{ marginTop: 8 }}>➕ Agregar planta</button>
                            )}
                          </div>
                        </div>
                      )
                ...
                return (
                  <>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                      {!loteBuilderOpen && (
                        <button type="button" className="full-form-btn" onClick={() => { setLoteBuilderOpen(true); setLoteStep('header'); setLoteAddingNewPlant(false) }}>🧺 Nueva compra</button>
                      )}
                      <button type="button" onClick={() => setIngresosMenuOpen(!ingresosMenuOpen)}>🔍 Buscar compra</button>
                    </div>
                    ...
                    {/* ---------- PASO 1: Cabecera de la compra (Proveedor + Nota) ---------- */}
                    {loteBuilderOpen && loteStep === 'header' && (
                      <div className="admin-form">
                        ...
                        <h3>Nueva compra</h3>
                        <input placeholder="Proveedor (opcional)" list="proveedores-list" value={loteProveedor} onChange={e => setLoteProveedor(e.target.value)} />
                        ...
                        <div className="admin-item-actions" style={{ flexDirection: 'column', gap: 8, marginTop: 10 }}>
                          <button
                            type="button"
                          ...
                          <button
                            ...
                            style={{ background: '#fff', color: '#b03434', padding: '10px 16px', borderRadius: 6, border: '1px solid #b03434', width: '100%' }}
                          >
                            Cancelar
                          </button>
                        </div>
                    ...
                    {loteBuilderOpen && loteStep === 'products' && (
                      <div className="admin-form">
                        ...
                        <div style={{ marginBottom: 16 }}>
                          <h4>Productos agregados ({loteLines.length})</h4>
                          {loteLines.length === 0 ? (
                            <p className="status-msg" style={{ margin: '4px 0' }}>Aún no has agregado productos. Busca una planta abajo para comenzar.</p>
                          ) : (
                        ...
                        {/* Buscador de plantas, prominente */}
                        <h4>Agregar planta a esta compra</h4>
                        <select
                          className="gallery-select"
                          value={loteLinePlantCategory}
                          onChange={e => { setLoteLinePlantCategory(e.target.value); setLoteAddingNewPlant(false) }}
                          style={{ fontSize: '1rem' }}
                        >
                          <option value="all">Todas las categorías</option>
                          {categories.map(c => <option key={c.id} value={c.id}>{c.emoji} {c.name}</option>)}
                        ...
                        <input
                          ...
                          onChange={e => { setLoteLinePlantSearch(e.target.value); setLoteAddingNewPlant(false) }}
                          style={{ fontSize: '1rem', padding: 10 }}
                        />

                        {/* Resultados de búsqueda / opción de planta nueva: ocultos una vez que hay una selección activa */}
                        {!lineForm.plant_id && !loteAddingNewPlant && (
                          <>
                            ...
                            <button
                              type="button"
                              onClick={() => setLoteAddingNewPlant(true)}
                              style={{ margin: '8px 0', background: 'none', border: '1px dashed #4a5d3a', color: '#4a5d3a', padding: '10px 12px', borderRadius: 6, cursor: 'pointer', width: '100%' }}
                            >
                              ＋ Es una planta nueva (no está en el catálogo)
                            </button>
                          </>
                        )}

                        {/* Formulario contextual: solo aparece tras seleccionar una planta (existente o nueva) */}
                        {(lineForm.plant_id || loteAddingNewPlant) && (
                          <div style={{ background: '#f3ecdd', padding: 12, borderRadius: 8, marginTop: 8, border: '1px solid #d8cdb0' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                              <strong>
                                {lineForm.plant_id
                                  ? `✓ ${plants.find(p => p.id === lineForm.plant_id)?.name || ''}`
                                  : '🌱 Registrando planta nueva'}
                              </strong>
                              <button
                                type="button"
                                onClick={() => { setLineForm({ plant_id: '', new_plant_name: '', new_plant_category: '', quantity: '', unit_cost: '', sale_price: '', file: null }); setLoteAddingNewPlant(false) }}
                                style={{ fontSize: '0.75rem', background: 'none', border: 'none', color: '#b03434', cursor: 'pointer' }}
                              >
                                ✕ Cambiar selección
                              </button>
                            </div>
                            ...
                            <button
                              type="button"
                              onClick={e => { if (addLineToLote(e)) { setLoteAddingNewPlant(false); setLoteLinePlantSearch(''); setLoteLinePlantCategory('all') } }}
                              style={{ background: '#4a5d3a', color: '#fff', padding: '10px 16px', borderRadius: 6, border: 'none', marginTop: 8, width: '100%' }}
                            >
                              ➕ Agregar a la lista
                            </button>
                          </div>
                        ...
                        {loteLines.length > 0 && (
                          <div className="admin-item-actions" style={{ marginTop: 18 }}>
                            <button
                              type="button"
                              onClick={saveLote}
                              disabled={savingLote}
                            ...
                            <button
                              type="button"
                              onClick={() => { setLoteBuilderOpen(false); setLoteLines([]); setLoteNota(''); setLoteProveedor(''); setLoteStep('header'); setLoteAddingNewPlant(false) }}
                        ...
                        return (
                          <div key={lote.id} className="admin-item lote-group">
                            <div className="admin-item-info">
                              ...
                              <div style={{ overflowX: 'auto', marginTop: 8 }}>
                                <table className="invoice-table" style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
                                  <thead>
                                    <tr style={{ background: '#f3ecdd' }}>
                                      <th style={{ padding: 4, border: '1px solid #ddd' }}>Foto</th>
                                      <th style={{ padding: 4, border: '1px solid #ddd' }}>Nombre</th>
                                  ...
                                  <tbody>
                                    {lineasConProrrateo.map(c => (
                                      <tr key={c.id}>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                          {c.image_url ? <img src={c.image_url} alt={c.plant_name} style={{ width: 32, height: 32, objectFit: 'cover', borderRadius: 4 }} /> : '—'}
                                          {editing && (
                                            <label className="file-label" style={{ display: 'block', marginTop: 2, fontSize: 10, background: 'transparent', color: 'inherit', border: '1px solid #ccc' }}>
                                              📷
                                              <input
                                                type="file"
                                                accept="image/*"
                                        ...
                                        <td style={{ padding: 4, border: '1px solid #ddd' }}>
                                          {editing ? <input defaultValue={c.plant_name} onBlur={e => updateCompraField(c, 'plant_name', e.target.value)} style={{ width: 90 }} /> : c.plant_name}
                                        </td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                          {editing ? <input type="number" defaultValue={c.quantity} onBlur={e => updateCompraField(c, 'quantity', e.target.value)} style={{ width: 45 }} /> : c.quantity}
                                        </td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>
                                          {editing ? <input type="number" step="0.01" defaultValue={c.unit_cost} onBlur={e => updateCompraField(c, 'unit_cost', e.target.value)} style={{ width: 60 }} /> : `$${Number(c.unit_cost).toFixed(2)}`}
                                        </td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${c._value.toFixed(2)}</td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${c._prorated.toFixed(2)}</td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${c._lineTotal.toFixed(2)}</td>
                                        <td style={{ padding: 4, border: '1px solid #ddd', textAlign: 'center' }}>
                                          <StatusChecklist
                                  ...
                                  <tfoot>
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Subtotal</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>${subtotal.toFixed(2)}</td></tr>
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 1</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>{editing ? <input type="number" step="0.01" defaultValue={lote.envio1 || 0} onBlur={e => updateLoteExtra(lote, 'envio1', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.envio1 || 0).toFixed(2)}`}</td></tr>
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Envío 2</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>{editing ? <input type="number" step="0.01" defaultValue={lote.envio2 || 0} onBlur={e => updateLoteExtra(lote, 'envio2', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.envio2 || 0).toFixed(2)}`}</td></tr>
                                    <tr><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}>Varios</td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}>{editing ? <input type="number" step="0.01" defaultValue={lote.varios || 0} onBlur={e => updateLoteExtra(lote, 'varios', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.varios || 0).toFixed(2)}`}</td></tr>
                                    <tr style={{ background: '#f3ecdd' }}><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd' }}><strong>Total</strong></td><td colSpan={4} style={{ padding: 4, border: '1px solid #ddd', textAlign: 'right' }}><strong>${totalLote.toFixed(2)}</strong></td></tr>
                                  </tfoot>
                                </table>
                              </div>
