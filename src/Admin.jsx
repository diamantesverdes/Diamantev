                        ...
                        return (
                          <div key={lote.id} className="admin-item lote-group">
                            <div className="admin-item-info">
                              ...
                              <div style={{ width: '100%', maxWidth: '100%', display: 'block', overflowX: 'auto', WebkitOverflowScrolling: 'touch', marginTop: 8, borderRadius: 6, border: '1px solid #ddd', boxSizing: 'border-box' }}>
                                <table className="invoice-table" style={{ borderCollapse: 'separate', borderSpacing: 0, width: 'max-content', minWidth: '100%', fontSize: 11.5 }}>
                                  <thead>
                                    <tr style={{ background: '#f3ecdd' }}>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', position: 'sticky', left: 0, width: 40, minWidth: 40, backgroundColor: '#f3ecdd', zIndex: 1, boxShadow: '2px 0 3px rgba(0,0,0,0.08)' }}>Foto</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', position: 'sticky', left: 40, width: 78, minWidth: 78, backgroundColor: '#f3ecdd', zIndex: 1, boxShadow: '2px 0 3px rgba(0,0,0,0.08)' }}>Nombre</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', width: 45, minWidth: 45 }}>Cant.</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', width: 60, minWidth: 60 }}>P.Unit</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', width: 60, minWidth: 60 }}>P.Total</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', width: 65, minWidth: 65 }}>Prorrateo</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', width: 60, minWidth: 60 }}>Total</th>
                                      <th style={{ padding: '4px 3px', border: '1px solid #ddd', width: 90, minWidth: 90 }}>Estado</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {lineasConProrrateo.map(c => (
                                      <tr key={c.id}>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'center', position: 'sticky', left: 0, width: 40, minWidth: 40, backgroundColor: '#fff', zIndex: 1, boxShadow: '2px 0 3px rgba(0,0,0,0.08)' }}>
                                          {c.image_url ? <img src={c.image_url} alt={c.plant_name} style={{ width: 28, height: 28, objectFit: 'cover', borderRadius: 4 }} /> : '—'}
                                          {editing && (
                                            <label className="file-label" style={{ display: 'block', marginTop: 2, fontSize: 9, background: 'transparent', color: 'inherit', border: '1px solid #ccc' }}>
                                              📷
                                              <input
                                                type="file"
                                                accept="image/*"
                                        ...
                                        <td style={{ padding: '3px', border: '1px solid #ddd', position: 'sticky', left: 40, width: 78, minWidth: 78, backgroundColor: '#fff', zIndex: 1, boxShadow: '2px 0 3px rgba(0,0,0,0.08)' }}>
                                          {editing ? <input defaultValue={c.plant_name} onBlur={e => updateCompraField(c, 'plant_name', e.target.value)} style={{ width: 70 }} /> : c.plant_name}
                                        </td>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'center' }}>
                                          {editing ? <input type="number" defaultValue={c.quantity} onBlur={e => updateCompraField(c, 'quantity', e.target.value)} style={{ width: 40 }} /> : c.quantity}
                                        </td>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>
                                          {editing ? <input type="number" step="0.01" defaultValue={c.unit_cost} onBlur={e => updateCompraField(c, 'unit_cost', e.target.value)} style={{ width: 55 }} /> : `$${Number(c.unit_cost).toFixed(2)}`}
                                        </td>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>${c._value.toFixed(2)}</td>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>${c._prorated.toFixed(2)}</td>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>${c._lineTotal.toFixed(2)}</td>
                                        <td style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'center' }}>
                                          <StatusChecklist
                                  ...
                                  <tfoot>
                                    <tr><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd' }}>Subtotal</td><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>${subtotal.toFixed(2)}</td></tr>
                                    <tr><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd' }}>Envío 1</td><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>{editing ? <input type="number" step="0.01" defaultValue={lote.envio1 || 0} onBlur={e => updateLoteExtra(lote, 'envio1', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.envio1 || 0).toFixed(2)}`}</td></tr>
                                    <tr><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd' }}>Envío 2</td><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>{editing ? <input type="number" step="0.01" defaultValue={lote.envio2 || 0} onBlur={e => updateLoteExtra(lote, 'envio2', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.envio2 || 0).toFixed(2)}`}</td></tr>
                                    <tr><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd' }}>Varios</td><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}>{editing ? <input type="number" step="0.01" defaultValue={lote.varios || 0} onBlur={e => updateLoteExtra(lote, 'varios', e.target.value)} style={{ width: 80 }} /> : `$${Number(lote.varios || 0).toFixed(2)}`}</td></tr>
                                    <tr style={{ background: '#f3ecdd' }}><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd' }}><strong>Total</strong></td><td colSpan={4} style={{ padding: '3px', border: '1px solid #ddd', textAlign: 'right' }}><strong>${totalLote.toFixed(2)}</strong></td></tr>
                                  </tfoot>
                                </table>
                              </div>
