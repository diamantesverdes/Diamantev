...
export default function App() {
    ...
    await processQueue({
      order: async (payload) => {
        const { data: lote, error } = await supabase
          .from('venta_lotes')
          .insert({ cliente: `${payload.customer_name} (${payload.customer_phone})` })
          .select()
          .single()
        if (error) throw error
        const items = payload.items.map(i => ({
          plant_id: i.isSeed ? null : i.id, seed_batch_id: i.isSeed ? i.seedBatchId : null, plant_name: i.name, quantity: i.quantity,
          motivo: 'Venta manual (con precio)', unit_price: i.price, lote_id: lote.id, status: 'pedido',
        }))
        await supabase.from('decrementos').insert(items)
        for (const item of payload.items) {
          if (item.isSeed) {
            await supabase.rpc('descontar_stock_semillas', { p_id: item.seedBatchId, p_qty: item.quantity })
            continue
          }
  ...
  async function loadData() {
    ...
    // Catálogo normal: solo las plantas que el administrador marcó en Galería (Modo Tienda Cliente).
    // Link compartido (?shared=1): solo las marcadas con "Compartir/Enviar", independiente de lo anterior.
    let plantsQuery = supabase.from('plants').select('*').order('name')
    plantsQuery = sharedMode
      ? plantsQuery.eq('shared_visible', true)
      : plantsQuery.eq('marcado_cliente', true)
    const { data: pls } = await plantsQuery
    // Semillas marcadas para cliente (vista pública seed_batches_public). Si la vista aún no existe, simplemente no se muestran.
    let seedItems = []
    if (!sharedMode) {
      const { data: sds } = await supabase.from('seed_batches_public').select('*')
      seedItems = (sds || []).map(sd => ({
        id: `seed:${sd.id}`,
        isSeed: true,
        seedBatchId: sd.id,
        name: `🌰 ${sd.nombre}`,
        category_id: sd.category_id || null,
        price: Number(sd.price) || 0,
        stock: Number(sd.stock) || 0,
        image_url: sd.image_url || null,
      }))
    }
    // Orden natural (Epidendrum 2 antes que 10); las semillas se ordenan por su nombre sin el 🌰.
    const sortName = p => (p.isSeed ? p.name.replace(/^🌰\s*/, '') : (p.name || ''))
    const allItems = [...(pls || []), ...seedItems].sort((x, y) => sortName(x).localeCompare(sortName(y), 'es', { numeric: true, sensitivity: 'base' }))
    const visibleCatIds = new Set(allItems.map(p => p.category_id))
  ...
  const heroPlants = plants.filter(p => p.image_url && p.coming_soon).slice(0, 8)
  ...
  async function sendOrder() {
    ...
    if (!isOnline()) {
      await addToQueue('order', {
        ...
        total,
        items: cart.map(i => ({ id: i.id, name: i.name, quantity: i.quantity, price: i.price, isSeed: !!i.isSeed, seedBatchId: i.seedBatchId || null })),
      })
      setCart([])
      setCustomerName('')
      setCustomerPhone('')
    ...
    const { data: lote, error } = await supabase
      .from('venta_lotes')
      .insert({ cliente: `${customerName} (${customerPhone})` })
      .select()
      .single()
    if (error) {
      alert('Hubo un error al registrar la venta. Intenta de nuevo.')
      setSending(false)
      return
    }
    const items = cart.map(i => ({
      plant_id: i.isSeed ? null : i.id,
      seed_batch_id: i.isSeed ? i.seedBatchId : null,
      plant_name: i.name,
      quantity: i.quantity,
      motivo: 'Venta manual (con precio)',
      unit_price: i.price,
      lote_id: lote.id,
      status: 'pedido',
    }))
    ...
    for (const item of cart) {
      if (item.isSeed) {
        // Las semillas se descuentan con una función segura en Supabase (no se abre permiso de edición público)
        await supabase.rpc('descontar_stock_semillas', { p_id: item.seedBatchId, p_qty: item.quantity })
        continue
      }
      await supabase
    ...
    const message = `Hola, quiero confirmar mi compra en 
  Diamantev (Venta #${lote.numero}):%0A%0A${lines}%0A%0ATotal: $${total.toFixed(2)}%0ANombre: ${customerName}%0ATeléfono: ${customerPhone}`
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${message}`, '_blank')
    setCart([])
    setCustomerName('')
    setCustomerPhone('')
    ...
    return (
      <div key={plant.id} className="card">
       <div className="card-img">
          ...
          {plant.image_url && <div className="watermark-overlay" aria-hidden="true" />}
     ...
     <div className="site-header">
      ...
      {heroPlants.length > 0 && (
        <div className="hero-carousel">
          ...
          <span className="hero-carousel-badge">🔜 Próximamente</span>
      ...
      {menuOpen && (
        <div className="cart-overlay menu-overlay-left" onClick={() => setMenuOpen(false)}>
...
>
  <svg viewBox="0 0 32 32" width="28" height="28" fill="#fff">
    <path d="M16.004 3C9.377 3 4 8.377 4 15.004c0 2.65.87 5.1 2.35 7.09L4.5 28l6.09-1.8a11.94 11.94 0 0 0 5.414 1.3h.005c6.627 0 12.004-5.377 12.004-12.004C28.013 8.377 22.636 3 16.004 3zm0 21.6a9.55 9.55 0 0 1-4.87-1.34l-.35-.207-3.61 1.07 1.08-3.52-.227-.36a9.56 9.56 0 0 1-1.47-5.12c0-5.293 4.31-9.6 9.6-9.6 5.293 0 9.6 4.31 9.6 9.6s-4.307 9.6-9.6 9.6zm5.29-7.19c-.29-.145-1.71-.845-1.976-.94-.266-.097-.46-.145-.653.145s-.75.94-.92 1.135c-.17.195-.34.218-.63.073-.29-.145-1.224-.451-2.332-1.437-.862-.769-1.444-1.72-1.614-2.01-.17-.29-.018-.447.127-.592.13-.13.29-.34.435-.51.145-.17.193-.29.29-.485.096-.194.048-.364-.024-.51-.073-.145-.653-1.575-.895-2.158-.236-.567-.477-.49-.653-.5-.169-.008-.363-.01-.557-.01-.194 0-.51.073-.777.363-.266.29-1.017.994-1.017 2.424 0 1.43 1.041 2.812 1.186 3.006.145.194 2.05 3.13 4.966 4.39.694.3 1.235.48 1.657.615.696.222 1.33.19 1.83.115.558-.083 1.71-.7 1.95-1.376.24-.677.24-1.257.169-1.376-.07-.12-.264-.194-.554-.34z"/>
      ...
      {isSearching ? (
        ...
        <>
          <div className="catalog-header-row">
            <h2 className="plants-title">🌿 Catálogo</h2>
            ...
            >
              {selectedCategoryIds.length === 0
                ...
                : selectedCategoryIds.length === 1
                  ? `${categories.find(c => c.id === selectedCategoryIds[0])?.emoji || '🌿'} ${categories.find(c => c.id === selectedCategoryIds[0])?.name || ''}`
                  : `${selectedCategoryIds.length} categorías seleccionadas`}
              {' '}▾
            </button>
            {categoryDropdownOpen && (
              <div className="category-dropdown-list">
                <button
                  ...
                  onClick={() => { setSelectedCategoryIds([]); setCategoryDropdownOpen(false) }}
                >
                  Todas las categorías
                </button>
                {categories.map(cat => (
                  <button
                    ...
                    onClick={() => { setSelectedCategoryIds([cat.id]); setCategoryDropdownOpen(false) }}
                  >
                    {cat.emoji || '🌿'} {cat.name}
                  </button>
                ))}
      ...
      {detailPlant && (
        <div className="cart-overlay menu-overlay-left" onClick={closeDetail}>
